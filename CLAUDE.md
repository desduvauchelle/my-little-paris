...
| `SITE_NOINDEX` | No | Server | `true` = site-wide noindex + disallow-all robots.txt (dev/staging). Remove to go live. |
...
**Meta title/description lengths are enforced.** The rendered title (including the ` | ${SITE_NAME}` brand suffix `buildPageMetadata` adds) must be **≤ 65 characters**, and the description must be **120–160 characters** — outside that, Google truncates or rewrites them and the site crawler flags `title_length` / `meta_description_length`. [src/app/page-metadata.unit.test.ts](src/app/page-metadata.unit.test.ts) asserts both for every marketing page; add new pages to its table. There is deliberately no per-page opt-out. Keep meta copy in dedicated `*.meta.title` / `*.meta.description` dictionary keys — do **not** reuse a page's on-screen subtitle (`*.sub`, `*.subtitle`), which is written to be short and will land well under the 120-char floor.
