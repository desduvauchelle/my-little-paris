...
import { PACKAGE_FORM_OPTIONS, PACKAGE_GROUPS, PARTY_DEPOSIT } from '@/data/party'
import {
...
export function PackageTabs({ dict }: { dict: Dictionary }) {
	...
	function handlePackageInquiry(packageName: string) {
		const optionPrefix = catered ? `${packageName} ` : `Venue Rental: ${packageName} `
		const option = PACKAGE_FORM_OPTIONS.find((item) => item.startsWith(optionPrefix)) ?? ''
		...
		<div>
				...
				{variant.packages.map((pkg) => {
					...
					>
						...
						<div className="card-body grid grid-rows-[auto_auto_auto_1fr_auto] gap-0">
							...
							<p className="text-xs leading-relaxed text-base-content/60 lg:min-h-12 mb-4">👥 {pkg.capacity}</p>

							<div>
								<p className="text-xs font-semibold tracking-widest uppercase text-base-content/50 mb-3">
									{dict['party.included']}
								</p>
							...
							<div className="card-actions flex-col gap-2 mt-6">
								...
								<BalloonCelebration>
									<button type="button" onClick={() => handlePackageInquiry(pkg.name)} className="btn btn-primary btn-block">
			...
			{showPrivateRoomDetails && (
				<section id="package-details" className="scroll-mt-24 mt-16" aria-labelledby="package-details-heading">
						...
						<table className="table table-fixed min-w-[64rem] align-top">
							...
							<thead>
								...
								{PRIVATE_ROOM_PACKAGE_DETAILS.map((details) => (
									...
									>
										<span className="block text-xs uppercase tracking-wide text-base-content/50">{details.level}</span>
					...
					<p className="text-xs text-base-content/50 text-center mt-4">{dict['party.details.note']}</p>
				</section>
			)}
